...
public class SeamCarver {
	...
	static String imgPath = "surfer.jpg";
	...
	static void remove_seam(ImageData img, int[] seam) {
		...
		for (int row = 0; row < height; row++) {
			...
			for (int newColumn = seam[row], origColumn = seam[row]
					+ 1; newColumn < newWidth; newColumn++, origColumn++) {
	...
	public static void main(String[] args) {
		...
		File polt_result = new File("Plot_Result.jpg");

		try {
			ImageIO.write(s.img, "jpg", result);

			ImageIO.write(plot.img, "jpg", polt_result);
		} catch (IOException e) {
