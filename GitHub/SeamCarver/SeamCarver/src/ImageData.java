...
public class ImageData {
	...
	// returns null if invalid indices are given, arrayindexoutofbounds caught
	// and
	...
	// returns pixel object with members red, green, blue as floating point
	// values
	// deprecated
	...
	// returns floating point value of green value, if out of bounds returns
	// 2.0f
	...
	// if invalid indices are given error is printed to sys.err (for debugging
	// purposes)
	// You can either generate a color using Pixel.getIntColor(float, float
	// float);
	...
	// transposes the image rotating it 90 degrees, running this twice returns
	// the original image.
	void TransPose() {
	...
	// remove one column from the left hand side of the picture plus any other
	// rows designated for trimming
	void trimLeft() {
		this.trimLeft += 1;
		trim();
	}

	// remove one row from the top side of the picture plus any rows designated
	// for trimming
	void trimTop() {
		this.trimTop += 1;
		trim();
	...
	// remove one column from the right hand side of the picture plus any other
	// rows designated for trimming
	void trimRight() {
		this.trimRight += 1;
		trim();
	...
	// remove one row from the bottom side of the picture plus any rows
	...
	// before running trim you can set how many rows to remove when you next
	// call it.
	...
	// before running trim you can set how many columns to remove when you next
	// call it.
	// if there is already trim values set these will add to it
